...
res2 <- pbmc.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_logFC)
# 105 marker genes found

x<-matrix(res2$gene, ncol=5, byrow=T)
mydata2<-data.frame(cluster=0:20, genes=matrix(res$gene, ncol=5, byrow=T))
write.csv(mydata,file="MyData2.csv")
